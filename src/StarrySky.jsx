...
const StarrySky = () => {
  ...
  useEffect(() => {
    ...
    const render = (time) => {
      ...
      // --- B. CONSTELLATIONS (THE DRAWING TRAIL) ---
      constellations.current = constellations.current.filter(
        (line) => line.life > 0
      );
      constellations.current.forEach((line) => {
        // Decrease this number to make it last LONGER. Increase to make it fade FASTER.
        line.life -= 0.002;
      ...
      shootingStars.current.forEach((s) => {
        s.x += s.vx;
        s.y += s.vy;
        s.life -= 0.01; // Shooting stars last about 1.6 seconds
